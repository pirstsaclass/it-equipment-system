...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 30, 2025 at 04:39 PM
...
CREATE TABLE `categories` (
  `id` int(11) NOT NULL,
  `categories_code` varchar(2) NOT NULL,
...
INSERT INTO `categories` (`id`, `categories_code`, `categories_name`, `is_active`, `sort_order`, `created_at`, `updated_at`) VALUES
...
CREATE TABLE `categories_items` (
  `id` int(11) NOT NULL,
  `categories_icode` varchar(2) NOT NULL,
  `categories_iname` varchar(255) NOT NULL,
  `is_active` tinyint(1) DEFAULT 1,
  `sort_order` int(11) DEFAULT 0,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
...
INSERT INTO `categories_items` (`id`, `categories_icode`, `categories_iname`, `is_active`, `sort_order`, `created_at`, `updated_at`) VALUES
...
CREATE TABLE `equipment` (
  ...
  `department_id` int(11) DEFAULT NULL,
  `responsible_person` varchar(255) DEFAULT NULL,
  `equipment_status` enum('อุปกรณ์ใหม่','อุปกรณ์เดิม','อุปกรณ์ใหม่-ซ่อมเสร็จ','อุปกรณ์เดิม-ซ่อมเสร็จ') NOT NULL DEFAULT 'อุปกรณ์ใหม่',
...
INSERT INTO `equipment` (`id`, `equipment_code`, `equipment_name`, `category_id`, `category_item_id`, `brand`, `model`, `serial_number`, `purchase_date`, `purchase_price`, `department_id`, `responsible_person`, `equipment_status`, `specifications`, `equipment_image`, `created_at`, `updated_at`) VALUES
...
INSERT INTO `maintenance` (`id`, `repair_code`, `equipment_code`, `report_date`, `problem_description`, `reported_by`, `assigned_technician`, `cost`, `repair_status`, `solution_description`, `completed_date`, `school_name`, `building_name`, `floor_name`, `room_name`, `created_at`, `updated_at`) VALUES
...
(7, '', 6, '2025-10-29', 'ค้างหน้า logo', 'moss', '', 0.00, 'ซ่อมเสร็จ', '', '0000-00-00', 'โรงเรียนนานาชาติวารีเชียงใหม่', 'ตึก9', 'ชั้น 2', 'ห้องสำนักงาน', '2025-10-29 08:10:11', '2025-10-30 03:37:32');
...
ALTER TABLE `building_floor_plans`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_plan` (`school`,`building`,`academic_year`);
...
ALTER TABLE `categories_items`
  ADD PRIMARY KEY (`id`),
  ADD KEY `categories_icode` (`categories_icode`) USING BTREE;
...
ALTER TABLE `classroom_equipment`
  ...
  ADD KEY `idx_school_building` (`school`,`building`,`floor`);
...
ALTER TABLE `equipment`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `equipment_code` (`equipment_code`) USING BTREE,
...
ALTER TABLE `maintenance`
  ADD PRIMARY KEY (`id`),
  ADD KEY `equipment_id` (`equipment_code`);
...
--
ALTER TABLE `categories_items`
  ADD CONSTRAINT `categories_items_ibfk_1` FOREIGN KEY (`categories_icode`) REFERENCES `categories` (`categories_code`) ON DELETE CASCADE;
...
--
ALTER TABLE `maintenance`
  ADD CONSTRAINT `maintenance_ibfk_1` FOREIGN KEY (`equipment_code`) REFERENCES `equipment` (`id`);
